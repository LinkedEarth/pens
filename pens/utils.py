...
def kernelCov(kernel,N):
    ''' Generates a covariance matrix from a given kernel
    
    Parameters
    ----------
    kernel : NumPy array, length l < N
        convolution kernel     
    
    N : int
        matrix dimension (it is square, so one dimension only)
    ...
    '''
    # if len(kernel) >= N:
    #     raise ValueError("kernel must be shorter than N")
    #     C = None
    # else:
    Cs = np.zeros([N,N])
    for ii in np.arange(N):
        Cs[ii,:] = np.convolve(np.eye(N)[ii,:],kernel,'valid')
    C = Cs.dot(Cs.T)

    return C

def scaleC(C,new_diag):
    '''
    Scale a covariance matrix C along rows and columns to have diagonal values equal to those in new_diag
    '''
    Cn = np.diag((new_diag/np.diag(C))**.5).dot(C).dot(np.diag((new_diag/np.diag(C))**.5))
    return Cn

def genGaussRV(C, p=1):
    ''' 
    Generates a Gaussian random p-vector from a covariance matrix (possibly rank-deficient)  
    '''
    [u,s,v] = np.linalg.svd(C,full_matrices=False)
    y = u.dot(np.diag(s**.5)).dot(np.random.randn(len(C),p))
    return y

def genGaussRV_Csqrt(Csqrt,p=1):
    ''' Generates a Gaussian random p-vector from a precomputed covariance sqrt
    '''
    y = Csqrt.dot(np.random.randn(len(Csqrt),p))
    return y
...
def kernFilt(kern, N, p = 1):
    """
    Given a smoothing kernel and vector of standard deviation s, generates a random p-vector consistent with both
    """
    lk = len(kern)
    dx = np.random.randn(N+lk+1,p)
    
    y = np.empty((N,p)) 
    for j in range(p):
        y = pyleo.utils.tsutils.standardize(np.convolve(kern,dx[:,j])[lk:-lk])
